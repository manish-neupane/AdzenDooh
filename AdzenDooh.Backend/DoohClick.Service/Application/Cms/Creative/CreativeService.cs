...
namespace AdzenDooh.Service.Application.Cms.Creative
{
    public class CreativeService(IDataAccessService _DataAccessService, IFileMetadataService _MetadataService) : ICreativeService
    {
        ...
        {
            string result = await _DataAccessService.ActionProcedure("inv.SpCreativeIns", JsonConvert.SerializeObject(param));
            return JsonConvert.DeserializeObject<List<MvCreative>>(result);
        }


        public async Task<GridResponse<MvCreative>?> GetGrid(MvParamOption<MvCreativeFilter> param)
        {
            string result = await _DataAccessService.RetrievalProcedure( "inv.SpCreativeSel", JsonConvert.SerializeObject(param));
