	...
	*/
	...
	BEGIN
	    ...
	    BEGIN TRY
	        ...
	        IF NOT EXISTS
	        (
	            SELECT 1 FROM inv.Screen AS s
	            INNER JOIN #ScreenOperatingHour AS ts ON s.Id = ts.ScreenId
	            WHERE s.IsDeleted = 0
	        )
	        BEGIN
	        ...
	        (
	            SELECT 1
	            FROM #ScreenOperatingHour AS a
	            INNER JOIN #ScreenOperatingHour AS b
	        ...
	        (
	            SELECT 1
	            FROM #ScreenOperatingHour AS ts
	            INNER JOIN inv.ScreenOperatingHour AS soh
	        ...
	        SET @Json = ISNULL(
	            (
	                ...
	                INNER JOIN #Inserted AS i ON soh.Id = i.Id
	                ORDER BY
	                    soh.DayOfWeek ASC,
	                    soh.StartTime ASC
